...
\indent
Radno okruženje podrazumijeva radnu mašinu sa instaliranim Linux operatinim sistemom, kao i paketima squashfs-tools i genisoimage:
\begin{lstlisting}[style=BashInputStyle]
...
Paket squashfs-tools implementira 2 funkcije koje se koriste u ovom radu a koje pruža SquashFS (\url{http://tldp.org/HOWTO/SquashFS-HOWTO/whatis.html}).
...
\indent
NodeJS je platforma "otvorenog koda" za izvršavanje JavaScript koda izvan okruženja web pretraživača. Neovisna je od vrste operativnog sistema i može se instalirati na Windows/Linux i MacOS operativne sisteme. JVM(JavaVirtualMachine) i JRE(JavaRuntimeEnvironment) i JDK(JavaDevelopmentKit) zajedno predstavljaju sličnost sa NodeJS, ali za Java programski jezik. Može se reći da NodeJS objedinjuje sve te stvari u jedan koncept, ali za JavaScript programski jezik.\\
NodeJS ima "event-driven" baziranu arhitekturu koja omogućava asinhroni ulaz/izlaz informacija. To ga čini pogodnim za razvijanje web aplikacija koje imaju mnoštvo ulaznih/izlaznih operacija, kao i za razvoj "real-time" mrežnih aplikacija i browser igrica.\\
...
\noindent
NodeJS Modul je kreiran od istog baznog modula kao i svi ostali moduli. To je ubuntu-18.04.4-desktop-amd64.iso datoteka:
\begin{lstlisting}[style=BashInputStyle]
...
\noindent
Funkcija unsquashfs iz squashfs-tools paketa omogućava raspakivanje filesytem.squashfs direktorija. Rezultat unsquashfs funkcije je squashfs-root direktorij te je on kopiran u edit direktorij. Direktorij edit će u nastavku rada biti iskorišten da unutar njega budu instalirani nodejs paketi:
\begin{lstlisting}[style=BashInputStyle]
...
\noindent
Funkciju iz squashfs-tools, mksquashfs, omogućava kompresovanje edit direktorija u novu filesystem.squashfs datoteku. U kodu ispod su navedene 3 primjera mksquashfs funkcije, pri čemu prva (komanda na liniji 2) daje najslabiju kompresiju, ali je najbrža. Druga komanda se duže izvršava ali je veći procenat kompresije u odnosu na prvu komandu. Dok je kod treće komande procenat kompresije najveći, a vrijeme izvršenja najduže:
\begin{lstlisting}[style=BashInputStyle]
...
\noindent
Generisanje .iso datoteke je urađeno unutar extract-cd direktorija. Prije upotrebe genisoimage funkcije za generisanje .iso datoteka izvršena je nekolicina konfiguracijskih modifikacija.
...
\indent
Za pokretanje kreirane .iso datoteke iskorišten je KVM alat. Najprije je kreiran virtuelni hard disk pomoću qemu-img komande.
\begin{lstlisting}[style=BashInputStyle]
cd ~
...
\indent
Unutar ove live instalacije je demonstrirana upotreba NodeJS-a kreiranjem jednostavne mrežne aplikacije.\\
Unutar pokrenute live distribucije sa preinstaliranim NodeJS bibliotekama pokrenuta je mrežna aplikacija putem NodeJS-a na vrlo jednostavan način:
\begin{lstlisting}[style=BashInputStyle]
...
http://localhost:1337
\end{lstlisting}
Za potrebe rada nije rađena modifikacija ove web aplikacije, ali moguće je iskoristiti aplikaciju kao bazu za nadograđivanje po želji. HTTP web server se kreira unutar index.js datoteke te bi početna modifikacija mogla biti nadogradnja ove datoteke sa dodatnim funkcionalnostima.\\
\noindent
